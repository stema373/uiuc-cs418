    ...
    for (uint32_t x = 0; x < width; ++x) {
      ...
      for (int s = 0; s < aa; ++s) {
        ...
        if (panorama) {
          ...
          double sy = (static_cast<double>(height) - 2.0 * (y + dy)) / denom;
          double r2 = sx * sx + sy * sy;
          if (r2 > 1.0)
            continue;
          dir = normalize(fwd * std::sqrt(1.0 - r2) + rightVec * sx + up * sy);
        } else {
          double sx = (2.0 * (x + dx) - static_cast<double>(width)) / denom;
          double sy = (static_cast<double>(height) - 2.0 * (y + dy)) / denom;
        ...
        if (dof && !fisheye && !panorama && lens > 0.0 && focus > 0.0) {
          ...
          Vec3 lensOffset = rightVec * (dxLens * lens) + up * (dyLens * lens);
        ...
        Ray ray{ro, rd};
        RayTraceResult res = ray_trace(ray, 0, bounces, 0, gi, spheres, suns,
                                       bulbs, planes, xyzs, tris, bvhRoot);
